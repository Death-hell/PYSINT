...
- Uses wordlist (LFI-wordlist.txt by default)
- Concurrent async requests
- Detects LFI via content indicators (e.g., /etc/passwd, root:, etc.)
...
async def test_lfi(
    ...
    Returns a dict with keys: param, url, status, result, reason
    result is one of: "vulnerable", "safe", "error"
    """
    async with sem:
        try:
            ...
            if any(ind in text for ind in indicators):
                if debug:
                    console.print(f"[DEBUG] LFI indicator matched for {full_url}")
                return {
                    "param": param,
                    "url": full_url,
                    "status": resp.status_code,
                    "result": "vulnerable",
                    "reason": "indicator_matched",
...
async def run_lfi_scan(
    ...
    console.print(f"\n[bold yellow]🔍 Testing {len(payloads)} LFI payloads on {len(parameters)} parameters ({total} requests)...[/bold yellow]\n")
    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=DEFAULT_TIMEOUT) as client:
        ...
        with Progress(
            ...
            task = progress.add_task("[green]Scanning LFI payloads...", total=total)
            for param in parameters:
                for payload in payloads:
                    coro = test_lfi(client, url, param, payload, sem, debug)
...
# ================== UTILS ==================
def save_results(results: list, prefix: str = "lfi", results_dir: Path = DEFAULT_RESULTS_DIR):
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="PYSINT LFI Scanner — Local File Inclusion Detector",
        epilog="Example: python3 LFI-Scanner.py --url 'https://example.com/page?file=index' --params file --max-payloads 50 --debug"
    )
    parser.add_argument("--url", "-u", help="Target URL with at least one query parameter (e.g., https://example.com/page?file=1)")
    parser.add_argument("--params", "-p", help="Comma-separated list of parameters to test (e.g., 'file,page,doc'). If omitted, uses all from URL.")
    parser.add_argument("--wordlist", "-w", default=DEFAULT_WORDLIST, help=f"Path to LFI payloads wordlist (default: {DEFAULT_WORDLIST})")
...
def main():
    ...
    results = asyncio.run(
        run_lfi_scan(url, parameters, wordlist_file, max_payloads, max_concurrent, debug)
    )
    ...
    # Save results
    json_path, csv_path = save_results(results, prefix="lfi")
    saved = []
    if json_path:
