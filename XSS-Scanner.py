#!/usr/bin/env python3
"""
XSS-Scanner.py - Cross-Site Scripting Scanner for PYSINT Suite
Author: Mael
License: MIT
Repository: ~/PyToolKit

Features:
- Scans URL parameters for XSS vulnerabilities via payload reflection
- Uses wordlist (XSS-wordlist.txt by default)
- Concurrent async requests
- Detects reflection of raw and URL-encoded payloads
- Saves results to JSON/CSV in ~/PYSINT/results
- CLI mode (--url, --params, etc.) + Interactive fallback
- Debug mode (--debug) to log payloads and response snippets
...
# ================== DEFAULTS ==================
DEFAULT_WORDLIST = "wordlist/XSS-wordlist.txt"
...
async def test_xss(
    ...
    Returns a dict with keys: param, url, status, result, reason, reflected_snippet, response_length
    """
    async with sem:
        try:
            parsed = urllib.parse.urlparse(url)
            query_params = urllib.parse.parse_qs(parsed.query)
            query_params[param] = payload  # set as single value
            new_query = urllib.parse.urlencode(query_params, doseq=True)
            full_url = f"{parsed.scheme}://{parsed.netloc}{parsed.path}?{new_query}"

            if debug:
                console.print(f"[DEBUG] Testing payload on {param}: {payload[:50]}...")

            resp = await client.get(full_url, timeout=DEFAULT_TIMEOUT)
            body = resp.text or ""

            # Check for reflection (raw or encoded)
            ...
            if payload in body:
                ...
                end = min(start + len(payload) + 40, len(body))
                snippet = body[start:end].replace('\n', ' ').strip()
                reflected = True
            elif encoded_payload in body:
                start = body.find(encoded_payload)
                end = min(start + len(encoded_payload) + 40, len(body))
                snippet = body[start:end].replace('\n', ' ').strip()
                reflected = True

            if reflected:
                if debug:
                    console.print(f"[DEBUG] Payload reflected in response: ...{snippet}...")
                return {
                    "param": param,
                    "url": full_url,
                    "status": resp.status_code,
                    "result": "vulnerable",
                    "reason": "payload_reflected",
            ...
            if debug:
                console.print(f"[DEBUG] Request error for {url} (param={param}, payload={payload[:30]}...): {e}")
            return {
                "param": param,
...
async def run_xss_scan(
    ...
    console.print(f"\n[bold yellow]🔍 Testing {len(payloads)} XSS payloads on {len(parameters)} parameters ({total} requests)...[/bold yellow]\n")
    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=DEFAULT_TIMEOUT) as client:
        ...
        with Progress(
            ...
            task = progress.add_task("[green]Scanning XSS payloads...", total=total)
            for param in parameters:
                for payload in payloads:
                    coro = test_xss(client, url, param, payload, sem, debug)
...
# ================== UTILS ==================
def save_results(results: list, prefix: str = "xss", results_dir: Path = DEFAULT_RESULTS_DIR):
    ...
    if results:
        try:
            with open(csv_path, "w", newline="", encoding="utf-8") as cf:
                fieldnames = ["param", "url", "status", "result", "reason", "reflected_snippet", "response_length"]
                ...
                for r in results:
                    writer.writerow({
                        ...
                        "result": r.get("result", ""),
                        "reason": r.get("reason", ""),
                        "reflected_snippet": r.get("reflected_snippet", ""),
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="PYSINT XSS Scanner — Cross-Site Scripting Detector",
        epilog="Example: python3 XSS-Scanner.py --url 'https://example.com/search?q=test' --params q --max-payloads 50 --debug"
    )
    parser.add_argument("--url", "-u", help="Target URL with at least one query parameter (e.g., https://example.com/search?q=test)")
    parser.add_argument("--params", "-p", help="Comma-separated list of parameters to test (e.g., 'q,search,id'). If omitted, uses all from URL.")
    parser.add_argument("--wordlist", "-w", default=DEFAULT_WORDLIST, help=f"Path to XSS payloads wordlist (default: {DEFAULT_WORDLIST})")
    ...
    parser.add_argument("--debug", action="store_true", help="Enable debug output (shows payloads and response snippets)")
...
def main():
    ...
    console.rule("[bold red]PYSINT XSS Scanner[/bold red]")
    ...
    results = asyncio.run(
        run_xss_scan(url, parameters, wordlist_file, max_payloads, max_concurrent, debug)
    )
    duration = time.time() - start_time

    # Display results
    table = Table(title="XSS Scan Results", show_lines=True)
    ...
    # Save results
    json_path, csv_path = save_results(results, prefix="xss")
    saved = []
    if json_path:
